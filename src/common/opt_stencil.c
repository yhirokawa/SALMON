...
void opt_stencil_body_c_( int            const           PNLx
                        , int            const           PNLy
                        , int            const           PNLz
                        , int            const           NLx
                        , int            const           NLy
...
#pragma novector
...
#endif
  {
    ...
    __m512i mxm = _mm512_loadu_prefetch_epi32(idx + (ix - 4 + NLx));
...
#ifndef ARTED_DOMAIN_POWER_OF_TWO
#ifdef TUNING_INDEX_CALC
    __m512i mym = _mm512_loadu_prefetch_epi32(idy + (iy - 4 + NLy));
...
#else
#ifdef TUNING_INDEX_CALC
        z0 = _mm512_load_prefetch_epi64(e + idz[iz - 4 + NLz]);
        z2 = _mm512_load_prefetch_epi64(e + idz[iz + 4 + NLz]);
#else
...
void opt_stencil_c_( double complex const* restrict tpsi
                   , double complex      * restrict htpsi
                   , int            const* restrict ipx_sta_
                   , int            const* restrict ipx_end_
                   , int            const* restrict ipy_sta_
                   , int            const* restrict ipy_end_
                   , int            const* restrict ipz_sta_
                   , int            const* restrict ipz_end_
                   , double         const* restrict V_local
                   , int            const* restrict ix_sta_
                   , int            const* restrict ix_end_
                   , int            const* restrict iy_sta_
                   , int            const* restrict iy_end_
                   , int            const* restrict iz_sta_
                   , int            const* restrict iz_end_
                   , int            const* restrict idx
                   , int            const* restrict idy
                   , int            const* restrict idz
                   , double         const* restrict lap0_
                   , double         const           lapt[restrict 12]
                   , double         const           nabt[restrict 12]
)
{
  const int    ipx_sta = *ipx_sta_, ipx_end = *ipx_end_;
  const int    ipy_sta = *ipy_sta_, ipy_end = *ipy_end_;
  const int    ipz_sta = *ipz_sta_, ipz_end = *ipz_end_;
  const int    ix_sta  = *ix_sta_,  ix_end  = *ix_end_;
  const int    iy_sta  = *iy_sta_,  iy_end  = *iy_end_;
  const int    iz_sta  = *iz_sta_,  iz_end  = *iz_end_;
  const double lap0 = *lap0_;

  const int PNLx = ipx_end - ipx_sta + 1;
  const int PNLy = ipy_end - ipy_sta + 1;
  const int PNLz = ipz_end - ipz_sta + 1;
  const int NLx  = ix_end - ix_sta + 1;
  const int NLy  = iy_end - iy_sta + 1;
  const int NLz  = iz_end - iz_sta + 1;

  opt_stencil_body_c_(PNLx,PNLy,PNLz,NLx,NLy,NLz,idx,idy,idz,lap0,V_local,lapt,nabt,tpsi,htpsi);
}
